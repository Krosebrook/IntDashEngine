    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
        ...
        <div className="p-6 bg-slate-800/50 flex justify-end gap-3">
          ...
          <button 
            onClick={() => onSave({ ...kpi, value: isNaN(Number(val)) ? val : Number(val), status })}
            className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-xl"
          >
            Save Changes
          </button>
        </div>
